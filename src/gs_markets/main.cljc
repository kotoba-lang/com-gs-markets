(ns gs-markets.main
  "Kotodama WASM entrypoint for the gs_markets clean-room actor (L4) — Clojure port.
  ...
  facts (`gs_markets.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gs_markets")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "gsmarket_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}         :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "gsmarket_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"gs_markets.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gs_markets-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gs_markets-compat/ready)
